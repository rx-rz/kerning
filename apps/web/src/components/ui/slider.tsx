...
function Slider({
  ...
  function handlePointerUp(event: React.PointerEvent<HTMLDivElement>) {
    ...
    if (snapTimer.current) clearTimeout(snapTimer.current)
    snapTimer.current = setTimeout(() => setIsSnapping(false), SNAP_DURATION_MS)
  }
  ...
  return (
    ...
    >
      <SliderPrimitive.Root
        ...
        className={cn(
          [
            "relative flex h-14 min-h-14 w-full touch-none items-center overflow-hidden rounded-lg select-none",
            "bg-component shadow-[inset_0_0_0_1px_var(--line-hair)] outline-none",
      ...
      >
        <SliderPrimitive.Track
          ...
          className="relative h-full w-full grow overflow-hidden rounded-[inherit] bg-component data-[orientation=vertical]:h-full data-[orientation=vertical]:w-full"
        >
            ...
            isEditing ? (
              ...
              >
                <input
                  ...
                  className="h-8 w-20 rounded-md bg-background/90 px-2 text-right font-mono text-xs font-semibold tabular-nums shadow-[inset_0_0_0_1px_var(--line-hair)] outline-none focus-visible:shadow-[0_0_0_3px_color-mix(in_srgb,var(--ring)_20%,transparent),inset_0_0_0_1px_var(--ring)]"
                />
              </form>
            ) : canEdit ? (
              <button
                type="button"
                className={cn(
                  "pointer-events-auto -mr-2 min-h-9 rounded-md px-2 font-mono text-xs font-semibold tabular-nums outline-none transition-[background-color,box-shadow]",
                  "cursor-text hover:bg-background/35 focus-visible:shadow-[0_0_0_3px_color-mix(in_srgb,var(--ring)_24%,transparent)]",
                )}
